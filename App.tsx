    ...
    <div className="font-sans text-gray-800 bg-brand-light selection:bg-brand-gold selection:text-white">
          ...
          <div className="flex-1 text-center lg:text-left text-white animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mt-2">
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group bg-brand-green text-white h-14 rounded-xl font-bold text-base border-2 border-brand-green hover:bg-green-600 hover:border-green-600 transition-all shadow-xl hover:shadow-green-500/30 transform hover:-translate-y-1 flex items-center justify-center gap-2 w-full sm:w-[260px] box-border"
              >
              ...
              <a 
                href="#servicos" 
                className="h-14 rounded-xl font-bold text-base border-2 border-white text-white hover:bg-white hover:text-brand-blue transition-all transform hover:-translate-y-1 flex items-center justify-center w-full sm:w-[260px] box-border"
              >
      ...
      <footer className="bg-brand-blue text-white pt-24 pb-10 border-t border-blue-900 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            {/* Quick Links */}
            <div>
              <h4 className="font-bold text-lg mb-6 text-brand-gold">Menu Rápido</h4>
              <ul className="space-y-4 text-blue-100/80">
                {NAV_LINKS.map(link => (
                  <li key={link.label}>
                    <a href={link.href} className="hover:text-white hover:translate-x-1 transition-all inline-block">{link.label}</a>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {/* Treatments */}
            <div>
              <h4 className="font-bold text-lg mb-6 text-brand-gold">Tratamentos</h4>
              <ul className="space-y-4 text-blue-100/80">
                {SERVICES.slice(0, 5).map(s => (
                  <li key={s.id}>
                    <button onClick={() => openServiceModal(s)} className="hover:text-white hover:translate-x-1 transition-all text-left">
                      {s.title}
                    </button>
                  </li>
                ))}
          ...
          <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-blue-200/60 font-light">
            <p>&copy; {new Date().getFullYear()} Odonto Harmony Eusébio. Todos os direitos reservados.</p>
            <p className="flex items-center gap-1">
              Desenvolvido com <span className="text-red-400">❤</span> e Tecnologia
            </p>
          </div>
        </div>
      </footer>
      ...
      {selectedService && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in-up">
          <div className="absolute inset-0 bg-brand-blue/90 backdrop-blur-sm" onClick={closeServiceModal}></div>
          <div className="bg-white rounded-[2rem] max-w-2xl w-full relative z-10 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="h-2 bg-gradient-to-r from-brand-blue via-brand-green to-brand-gold"></div>
            
            <button 
              onClick={closeServiceModal}
              className="absolute top-6 right-6 bg-gray-100 hover:bg-gray-200 p-2 rounded-full transition-colors z-20"
            >
              {React.createElement(ICONS['close'], { className: "w-6 h-6 text-gray-600" })}
            </button>
            ...
            <div className="p-8 overflow-y-auto">
              ...
              <h3 className="text-3xl font-bold text-brand-blue mb-2">{selectedService.title}</h3>
              ...
              <div className="bg-brand-light p-6 rounded-2xl border border-brand-blue/5">
                <h4 className="font-bold text-brand-blue mb-4">Por que escolher este tratamento?</h4>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3 text-gray-600">
                    <svg className="w-5 h-5 text-brand-green shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    <span>Tecnologia de ponta e materiais de alta durabilidade.</span>
                  </li>
                  <li className="flex items-start gap-3 text-gray-600">
                    <svg className="w-5 h-5 text-brand-green shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    <span>Conforto e segurança durante todo o procedimento.</span>
                  </li>
                  <li className="flex items-start gap-3 text-gray-600">
                    <svg className="w-5 h-5 text-brand-green shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    <span>Acompanhamento personalizado pós-tratamento.</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-end gap-4">
              <button onClick={closeServiceModal} className="px-6 py-3 rounded-xl font-bold text-gray-500 hover:text-gray-800 transition">
                Fechar
              </button>
              <a 
                href={getWhatsAppLink(`Olá, gostaria de saber mais sobre ${selectedService.title}`)}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-brand-green text-white px-8 py-3 rounded-xl font-bold shadow-lg hover:bg-green-600 hover:shadow-green-500/30 transition-all transform hover:-translate-y-1 flex items-center gap-2"
              >
